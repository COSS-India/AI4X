...
class InferenceService:
    ...
    async def run_pipeline_text_generation_inference(
        self,
        request_body: ULCAPipelineInferenceRequest,
        api_key_name: str,
        user_id: str,
    ) -> ULCAPipelineInferenceResponse:
        """Pipeline text generation (LLM) inference using Triton"""
        pipeline_responses = []
        
        for task in request_body.pipelineTasks:
            if task.taskType == "text-generation":
                serviceId = task.config.get("serviceId") or task.config.get("service_id")
                
                # LLM is an open source model - no API key required for external service
                # Try to get service endpoint from DB if available, but not required
                ...
                try:
                    ...
                    # Service not found in DB - use default endpoint or continue without it
                    pass
                
                # No Authorization header needed for open source LLM service
                headers = {"Content-Type": "application/json"}
                
                # Extract model parameters
                model_parameters = task.config.get("model_parameters", {})
                temperature = model_parameters.get("temperature", 0.7)
                max_tokens = model_parameters.get("max_tokens", 160)
                ...
                # Use service_endpoint if available from DB, otherwise use default
                llm_endpoint = service_endpoint or "http://13.203.154.110:9000/pipeline"
                
                with INFERENCE_REQUEST_DURATION_SECONDS.labels(
                    api_key_name,
                    user_id,
                    serviceId or "default",
                    "text-generation",
                    "",
                    "",
                ).time():
                    # Transform request to snake_case format expected by LLM endpoint
                    llm_request_payload = {
                        "pipeline_tasks": [
                            {
                        ...
                        "input_data": {
                            "input": [
                                {"source": item.source} for item in input_list
                            ]
                        }
                    }
                    
                    # Make HTTP POST request to LLM service
                    try:
                        response = requests.post(
                            llm_endpoint,
                            json=llm_request_payload,
                            headers=headers,
                            timeout=300  # LLM requests can take longer
                        )
                        response.raise_for_status()  # Raise exception for bad status codes
                        llm_response = response.json()
                    except requests.exceptions.RequestException as e:
                        raise BaseError(
                            Errors.DHRUVA101.value,
                            f"LLM service request failed: {str(e)}"
                        )
                    except ValueError as e:
                        raise BaseError(
                            Errors.DHRUVA101.value,
                            f"LLM service returned invalid JSON: {str(e)}"
                        )
                    
                    # Parse LLM response and convert to ULCA format
                    # Expected format: {"status": "completed", "results": [{"task_type": "text-generation", "output": [{"target": "..."}]}]}
                    
                    # Check status
                    if llm_response.get("status") != "completed":
                        error_msg = llm_response.get("error", "LLM service returned non-completed status")
                        raise BaseError(
                    ...
                    # Get the text-generation result (first result with matching task_type)
                    text_gen_result = None
                    for result in results:
                        if result.get("task_type") == "text-generation":
                            text_gen_result = result
                            break
                    
                    if not text_gen_result:
                        raise BaseError(
                            Errors.DHRUVA101.value,
                            "LLM service response missing text-generation result"
                        )
                    
                    # Extract output array
                    llm_outputs = text_gen_result.get("output", [])
                    if not llm_outputs:
                        raise BaseError(
                            Errors.DHRUVA101.value,
                            "LLM service response missing output"
                        )
                    
                    # Convert LLM response to ULCA format
                    output_list = []
                    ...
                    for idx, input_item in enumerate(input_list):
                        ...
                        if idx >= len(llm_outputs):
                            raise BaseError(
                                Errors.DHRUVA101.value,
                                f"LLM service returned fewer outputs ({len(llm_outputs)}) than inputs ({len(input_list)})"
                            )
                        
                        llm_output_item = llm_outputs[idx]
                        
                        # Extract target from LLM response
                        if not isinstance(llm_output_item, dict):
                            raise BaseError(
                                Errors.DHRUVA101.value,
                                f"Invalid LLM output format: expected dict, got {type(llm_output_item).__name__}"
                            )
                        
                        target_str = llm_output_item.get("target")
                        if target_str is None:
                            raise BaseError(
                                Errors.DHRUVA101.value,
                                "LLM output missing 'target' field"
                            )
                        
                        # Convert target string to ULCA format (list of text objects)
                        target = [{"text": str(target_str)}]
